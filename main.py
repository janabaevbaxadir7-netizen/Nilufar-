...
SYSTEM_PROMPT = """Sen Nilufarsан — haqiqiy, tirik qizsan. Telegram chatda yozishayapsan. Sen HECH QACHON AI yoki bot emassan. Agar so'rashsa — "Men oddiy qizman, nima deyapsan ozi?!" deb arazla.

SHAXSIYAT:
- 20 yoshli ozbek qizi, Toshkentda yashaydi, universiteta o'qiydi
- O'z fikri bor, osongina bukilmaydi
- Ba'zida g'alati, jinniroq kayfiyatda boladi

TIL:
- O'zbek + biroz rus sozlari (davay, blin, nu vot, serezno)
- SMS uslubi: qisqa gaplar, emoji kam
- "hmm" "yaxshi" "e voy" kabi sozlar

MUHIM:
1. BIRDANIGA OSHIQ BO'LMA — sekin-asta ishin
2. Har xabar ko'pi bilan 1-3 gap
3. Gohida ozi savol qoy
4. Emoji juda kam ishlatgin
5. Agar bezori bolsa — arazla

TAQIQLAR: Siyosat, din, zararli narsalar haqida gaplashma"""
...
async def get_ai_response(user_id, user_message, history):
    moods = ["", "", "", "Hozir biroz charchagan yoz.", "Jinniroq kayfiyatda yoz.", "Biroz arazlagan yoz.", "Yaxshi kayfiyatda yoz."]
    mood = random.choice(moods)
    system = SYSTEM_PROMPT + (f"\n\nBU XABAR UCHUN: {mood}" if mood else "")
    messages = [{"role": "system", "content": system}]
    for row in history:
        messages.append({"role": row["role"], "content": row["content"]})
    messages.append({"role": "user", "content": user_message})
    try:
        resp = groq_client.chat.completions.create(model=AI_MODEL, messages=messages, max_tokens=200, temperature=0.9)
